...
Assert-Match $files.PublicHeader 'XDOWS_SECURITY_PROTOCOL_VERSION\s+7u' 'protocol v7'
...
Assert-Match $files.Bridge 'private\s+SafeFileHandle\?\s+_eventHandle' 'dedicated blocking event handle'
Assert-Match $files.Bridge 'TryGetNextEvent\(\)(?s:.*?)DeviceIoControlNoInput\(_eventHandle!' 'event pump uses dedicated device handle'
Assert-Match $files.Bridge 'SubmitDecision\(XdowsSecurityDecision decision\)(?s:.*?)DeviceIoControlNoOutput\(decision,\s*DriverProtocol\.SubmitDecision\)' 'decisions remain on control handle'
Assert-Match $files.Bridge 'SubmitDecision\(decision\);(?s:.*?)afterDecisionSubmitted\(driverEvent,\s*decision,\s*token\)' 'post-decision work starts after kernel submission'
...
Assert-Match $files.Protection 'QueueQuarantineAfterBlock\(driverEvent,\s*imagePath,\s*detectionName\)' 'process quarantine queued after block'
Assert-Match $files.Protection 'QueueQuarantineAfterBlock\(driverEvent,\s*filePath,\s*detectionName\)' 'file quarantine queued after block'
Assert-Match $files.Protection 'RunQuarantinePumpAsync(?s:.*?)await\s+QuarantineManager\s*\.AddToQuarantine' 'bounded post-decision quarantine worker'
Assert-NotMatch $files.Protection 'HandleFileEventAsync(?s:.*?)bool\s+quarantineSucceeded\s*=\s*await\s+QuarantineManager' 'file decision waiting for quarantine'
Assert-Match $files.Protection 'Block decision submitted to kernel' 'kernel block submission timing log'
...
Assert-Match $files.Protection 'Connecting DriverBridgeClient(?s:.*?)Creating NativeModelScanner(?s:.*?)NativeReady(?s:.*?)RegisterProtectedProcess\(\)' 'native model initialization before self-protection registration'
Assert-Match $files.Protection 'if\s*\(!_scanner\.NativeReady\)(?s:.*?)InitializationError' 'native model startup readiness rejection'
...
Assert-Match $files.InterceptCode 'TaskCompletionSource<string>\(TaskCreationOptions\.RunContinuationsAsynchronously\)' 'asynchronous intercept-window continuation'
Assert-Match $files.InterceptCode 'cancellationToken\.Register(?s:.*?)TrySetResult\("Timeout"\)(?s:.*?)DispatcherQueue\.TryEnqueue' 'timed-out intercept window cancellation'
