...
if ($public -notmatch 'XDOWS_SECURITY_PROTOCOL_VERSION\s+3u' -or
    $protocol -notmatch 'ProtocolVersion\s*=\s*3;') {
    throw "Protocol v3 is not mirrored."
}
if ($public -notmatch 'ULONG\s+FileProtectionEnabled;\s*ULONG\s+SelfProtectionEnabled;\s*ULONG\s+ProtectedProcessId;\s*ULONG\s+ActiveModules;' -or
    $protocol -notmatch 'uint\s+FileProtectionEnabled;\s*public\s+uint\s+SelfProtectionEnabled;\s*public\s+uint\s+ProtectedProcessId;\s*public\s+uint\s+ActiveModules;') {
